...
import type { SqlTransactionInterface, SqlPoolInterface, sqlPool } from "./sql";
...
namespace app {
	...
	export interface SqlTransaction extends SqlTransactionInterface {
	}

	export interface SqlPool extends SqlPoolInterface {
	}
	...
	export interface Config {
		...
		/**
		 * Configuration used to create the connection pool from where all SQL Server connections are served when using `app.sql`.
		 * 
		 ...
		 * ```
		 * 
		 * Refer to https://www.npmjs.com/package/mssql for more information on the available options.
		 */
...
interface App {
	...
	/**
	 * Provides a way to connect to the database, as specified by `config.sqlConfig`.
	 * 
	 * If `config.sqlConfig` is not provided, `app.sql` will be `null`.
	 */
	sql: app.SqlPool;
